...
public class CadastrarReservaService : ICadastrarReservaService
{
    ...
    public async Task Handle(CadastrarReservaCommand command)
    {
        try
        {
            var reserva = ReservaDb.Create(command.ClienteId, command.DataInicial, command.DataFinal, command.QuantidadeDiarias, command.QuantidadePets, command.ValorTotal, command.Observacoes, command.PetIds);
